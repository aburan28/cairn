...
## What proofwork already has

| Piece | proofwork Stage 0 |
|---|---|
...
| Agent surface | `proofwork-mcp`: list / get / score / frontier / submit / audit |
...
## Gaps (proofwork → ecdsa.fail-type)
...
3. **External promotion API.** ecdsa.fail promotion is “beats current best” via
   their API. proofwork settlement is local log + ratchet + citation flow. No
...
| Thin ecdsafail ↔ proofwork adapter | implemented (`adapter.sh`, `score_to_artifact.py`) |
| MCP wiring | natural: existing tools; `mcp-score.sh` smoke |
| Pin real `benchmark.sh` / submit to api.ecdsa.fail from proofwork | **not** implemented — blockers below |
...
2. Must pass `ecdsafail run` (full sim). Declared metrics alone are worthless on
   their API.
3. Must call `ecdsafail submit` with `--note-file` and `--model`; proofwork
...
5. Embedding their harness as a proofwork verifier would need a pinned, sandboxed
