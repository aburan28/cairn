...
objective for this instance -- objective-certicom-eccp131.json -- pays once, for `k`; this one pays
...
def check(artifact: dict) -> tuple[bool, str]:
    ...
    for index, element in enumerate(dps):
        ...
        seen.add(point)
    return True, f"verified: {len(dps)} distinguished points of the ECCp-131 walk (about 2^{DP_BITS} steps each)"


# -- the pinned instance and walk ----------------------------------------------
#
# Identical to checkers/eccp131_rho_dp.py: the same curve, P, Q, n, DP_BITS
