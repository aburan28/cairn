...
if [ "${#sigs[@]}" -lt "$QUORUM" ]; then
  echo "only ${#sigs[@]} relay(s) answered for round $round, need $QUORUM." >&2
...
echo "${#sigs[@]} relays agree on round $round; the pairing check is cairn's, below" >&2
