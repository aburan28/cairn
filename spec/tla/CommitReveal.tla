...
(*                                                                          *)
(* The implementation does not settle at the boundary -- it waits           *)
(* `FINALITY_EPOCHS` further epochs, so that a node which heard about this  *)
(* epoch late still holds every earlier one before it drains anything (see  *)
(* docs/design/settlement-convergence.md). That delay is deliberately NOT   *)
(* modelled here, and the omission is sound rather than convenient: this    *)
(* module has one node, and the delay changes only *when* a batch settles,  *)
(* never which claims are in it or how they are ordered. The property being *)
(* checked is the order, and adding a counter would multiply the state      *)
(* space to say nothing new.                                                *)
(*                                                                          *)
(* What the delay is really about is two nodes disagreeing, which no TLA+   *)
(* module here models -- there is no second node to disagree with. That     *)
(* case is covered by `tests/simulation.rs`, which runs many interleavings  *)
(* of the real code rather than all interleavings of an abstraction.        *)
Tick ==
    /\ epoch < MaxEpoch
