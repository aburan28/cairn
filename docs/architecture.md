...
verified results are the unit of account, and the whole thing coordinates around
shared objectives without a central lab.
...
One separation generalizes across every research harness worth copying, and the
failure it prevents is universal: **the agent that produces a result is never
