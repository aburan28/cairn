...
| **verifier-tree divergence** — two nodes with different `--root` contents reach different verdicts on the same claim | partial. Pinned code is addressed by path against a local root and only *checked* against its declared hash, so a node without the file returns `Unavailable` rather than diverging — which is the `Unavailable`-never-settles rule earning its keep. But it means public verifiability depends on out-of-band file distribution: a copy of the log is not sufficient to re-derive a settled result. Needs content-addressed blobs | partial |
| **eviction of a live evaluator** — `store gc` reclaims the only local copy of an evaluator an open objective depends on | none. The quota's two classes are the log (pinned) and everything under `cache/` (reclaimable); a blob that makes an objective verifiable is neither. The node then answers `Unavailable` for work it is paid to check, which under availability sampling is a slash | not handled |
