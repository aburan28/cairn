...
| **settlement-order divergence** — two honest nodes holding the same records pay the same claims in a *different order*, and neither can tell | **not handled**, and now proven rather than suspected: two `#[ignore]`d tests in `tests/p2p_convergence.rs` fail on demand, showing six claims settled in near-reversed order by two nodes given identical inputs. The batch sort key is `H(beacon(epoch, anchor) ‖ commitment_hash)`; epoch and commitment hash converge, and the **anchor does not** — it is a local ledger entry's hash, which covers `seq`, `prev` and the local write time. Distinct from **settlement reordering** below, which is a sequencer choosing an order on purpose; this is two honest nodes reaching different ones with no adversary at all, and **both logs audit clean**, because each is internally consistent. A single-operator deployment is unaffected — one log, one order — and every multi-operator story is blocked on it. Fix, cost and migration in [design/settlement-convergence.md](design/settlement-convergence.md) | **not handled** |
