...
| **citation-flow dilution** — split one improvement into many to starve the contributor you built on | **partial.** The default rule still decays δ per *hop*, so slicing is free in direct reward (telescoping) and strictly profitable in flow: 256 slices drive the upstream contributor's citation flow to *zero*, leaving only their own direct reward, and invert the payout result the README uses to argue publishing pays. Pinned in `tests/citation_flow.rs`. The fix is **built and proven** as `attribution::payouts_weighted` — δ split among all transitive ancestors weighted by each one's settled reward, which on a ratchet is the progress it moved. Five tests pin it: exact integer conservation, a downstream citer's contribution bounded within 10% however finely the middle is chopped, the slicer's gain converging under 1% between 16 and 256 slices, identity-blindness (so there is no sybil version), and zero-reward ancestors unable to dilute. It is **not yet the default**: switching the money path moves the conformance vectors and must land in one piece across both implementations, or they disagree about what everyone is owed. See [design/citation-flow-dilution.md](design/citation-flow-dilution.md) | partial |
...
| **plaintext swarm transport** — read a `swarm::tcp` transfer off the wire | `swarm::tcp` has no handshake and no AEAD, and is behind the off-by-default `insecure-swarm-tcp` feature so it cannot be built into a binary by accident. The state machines it drives are pure and always compiled; only the socket-facing part is gated. This is an unfinished merge rather than a design: `swarm` peer records carry an ed25519 identity while the encrypted transport needs a 261,120-byte McEliece key, so closing it means folding the stack onto `p2p`'s identity | partial |
| **what at-rest encryption does not cover** — read the data directory and learn more than the sealed log gives up | `store::atrest` seals the log line-wise and **only the log**. Two things beside it are plaintext: the blob store, whose filenames *are* content addresses, so the disk discloses which objectives this node works on — the same signal `p2p::code` refuses to publish on the wire and the DHT round is careful not to reintroduce; and the population file the daemon writes with `--population`, which is canonical JSON of gossiped candidates. Neither leaks a secret exactly — pinned checkers are in the log and fetchable, and gossiped candidates were shared with peers by design — so what leaks is *which* rather than *what*. It is still an asymmetry worth closing or deciding against on the record | not handled |
