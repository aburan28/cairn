...
| **forged submission over HTTP** — POST a claim under somebody else's `submitter` | **not handled**, and the most important open item. `submitter` is a self-declared string; nothing binds it to a key, so anyone reaching `POST /submit` can submit as anyone. The queue limits the damage to *proposals* -- the operator's rules engine still re-derives every admission rule, so a forged submission cannot bypass verification or citation rules -- but citation flow pays a *name*, and a name anybody can claim is an open till. Fix: an ed25519 signature over the commitment, keyed to the submitter. Until then a network with real value must not accept submissions from strangers | **not handled** |
