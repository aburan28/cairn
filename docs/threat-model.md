...
| **availability sybils** — many identities, one disk | **not handled.** A fixed pot bounds a funder's cost however many nodes appear, but nothing prices identity: ten keys behind one disk answer ten samples from one copy and take ten shares. Weighting stops one identity multiplying itself through extra promises; it cannot stop ten identities. The answer is a bond (`incentive::NodeParams::stake`), which the roadmap places at Stage 2 — **so an availability pool should not carry real money until it exists.** The mechanism is complete and the pricing is not | open |
| **availability answer replay** — reuse one entry forever, or across auditors | The sampled index is drawn from `beacon(epoch, anchor)`, so it moves each epoch; the anchor is bounded by the answer's own **position** in the log, so later appends cannot move it under a written answer; and it uses the `EPOCH_SECONDS` constant, never the `PROOFWORK_EPOCH_SECONDS` override, so the same log audits identically whatever the auditor's environment. The last two were real defects, measured at six failures in ten before the fix, and both read as flakiness rather than as bugs | handled |
...
| **settlement-order divergence** — two honest nodes holding the same records pay the same claims in a *different order*, and neither can tell | **partial.** Was real and is fixed for the deterministic case. The batch sort key is `H(beacon(epoch, anchor) ‖ commitment_hash)`; epoch and commitment hash always converged, and the **anchor did not** — it was a local ledger entry's hash, covering `seq`, `prev` and the local write time, so two nodes with byte-identical records settled six claims in near-reversed order while **both logs audited clean**. The anchor is now the head of an **epoch chain**: each link commits to the previous link, the epoch, and that batch's *sorted* claim ids, so it is content-only and equal on every node with the same batches — convergence by induction on the epoch. Both implementations derive it and `tests/p2p_convergence.rs` pins it. **Two cases remain open, and the first was found by adversarially reviewing this very fix rather than by the tests shipped with it.** (1) **Drain sequence**: a node that learns of later work first drains the later epoch first, so its batch for the *earlier* epoch anchors on the later epoch's link — two nodes then hold the same claims, settle the same claims, audit clean, and pay both epochs in the opposite order (`draining_epochs_in_a_different_sequence_forks_the_chain`). The chain removed the dependence on the ledger *envelope*, not on drain order. (2) **Partial view at drain time**: a node that settles an epoch before sync delivers a claim belonging to it writes a different link and diverges permanently. The fix for that is a finality delay or reorgs, and is not built. Distinct from **settlement reordering** below, which is a sequencer choosing an order on purpose. See [design/settlement-convergence.md](design/settlement-convergence.md) | partial |
...
| **what at-rest encryption does not cover** — read the data directory and learn more than the sealed log gives up | **decided, and partly closed.** `store::atrest` seals the log line-wise and only the log. That boundary is now a decision on the record rather than an accident: the blob store and the population file stay plaintext because every byte *and every name* in them is something the node hands to any peer that asks, so a stolen disk yields nothing the network does not give away for free. What still leaks is *which* rather than *what* — the set of blobs discloses which objectives this node works on, without the adversary having to ask a peer and be observed doing it. `HMAC(key, address)` filenames would close it and were rejected: they cost the property that the name *is* the hash, which is what lets a read re-hash and refuse mismatched bytes with no second index. `store::exposure` classifies every file in a store and `store status` exits 1 on anything unaccounted for, so a future plaintext artifact trips a check instead of quietly widening this row | partial |
