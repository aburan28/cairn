...
| **log rollback/fork** — publish a shorter or alternate internally valid chain | daemon checkpoints sign height, head, and Merkle root with a separately pinned ML-DSA-65 root key; readers must retain and verify the checkpoint key | partial; reader CLI still open |
