...
impl<const LIMBS: usize, const N: usize, MOD: ConstMontyParams<LIMBS>> CsidhParams<LIMBS, N, MOD> {
    ...
    ///
    /// ```
    /// // Path adjusted by the vendoring: upstream reads `use csidh::{...}`,
    /// // which named a crate that is now a module.
    /// use proofwork::crypto::csidh::{
    ///     impl_modulus, ConstMontyForm, CsidhParams, Uint
    /// };
    ///
    /// # fn main() {
