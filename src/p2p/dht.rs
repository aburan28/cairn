...
//! of closing it this way is that provider knowledge spreads one hop rather than
//! arbitrarily far — which costs nothing today, because the multi-hop driver
//! that would carry it further is not built. It is the reason to prefer signed
//! records if the network outgrows this.
//!
//! # Determinism
//!
...
pub enum DhtMessage {
    ...
    /// "Who holds this blob?" Keyed by content address, not by an opaque id, so
    /// the key is one anybody can compute from the log.
    GetProviders { address: String },
    /// Holders if any are known, and closer contacts either way.
    ///
    /// Both in one message on purpose: a lookup that got holders is finished, and
    /// one that did not needs somewhere to go next. Splitting them would cost a
    /// round trip on every miss, which is the common case near the start.
    Providers {
        address: String,
        holders: Vec<Holder>,
...
impl DhtMessage {
    pub fn to_value(&self) -> Value {
        match self {
            ...
            DhtMessage::GetProviders { address } => obj! {
                "t" => Value::string("dht_get_providers"),
                "address" => Value::string(address.clone()),
            },
            DhtMessage::Providers {
                ...
                "address" => Value::string(address.clone()),
                "holders" => Value::Array(holders.iter().map(Holder::to_value).collect()),
                "closer" => Value::Array(closer.iter().map(PeerContact::to_value).collect()),
            },
    ...
    pub fn from_value(value: &Value) -> Result<DhtMessage, DhtError> {
        ...
        let address = |field: &str| -> Result<String, DhtError> {
            let text = value
                .get(field)
                .and_then(Value::as_str)
                .ok_or_else(|| bad(format!("{field} is not a string")))?;
        ...
        match value.get("t").and_then(Value::as_str) {
            ...
            Some(kind @ ("dht_ask" | "dht_tell")) => {
                let items = array("addresses", MAX_ASK_ADDRESSES)?;
